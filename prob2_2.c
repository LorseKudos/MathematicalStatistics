...
int main(int argc, char *argv[]){
  ...
  for(k=0;k<NUMBER;k++){
    ...
    i *= a;
    u_2 =  ((double) i) / (pow(2,32) - 1) ;

    x[k] = u_1;
    if(atoi(argv[1]) == 1){
      y[k] = u_2; //変数XとYは独立な一様乱数
    }else if(atoi(argv[1]) == 2){
      y[k] = x[k] + u_2; //Z = X + Y,今y[k]は変数Zを表す
    }
  ...
  p = fmin(1 - Pr(t,NUMBER-2), Pr(t,NUMBER-2));
